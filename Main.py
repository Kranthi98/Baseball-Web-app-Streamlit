...
with col1:
    ...
    #if fg == "Pitch_Splits":
    pitcher1_df = (pb.statcast_pitcher(player_id = pb.playerid_lookup(name_split(pitcher1)[1].strip(),name_split(pitcher1)[0])["key_mlbam"].values[0],
...
with col2 : 
    ...
    #if fg == "Pitch_Splits":
    pitcher2_df = (pb.statcast_pitcher(player_id = pb.playerid_lookup(name_split(pitcher2)[1].strip(),name_split(pitcher2)[0])["key_mlbam"].values[0],
                start_dt = start_date, end_dt = end_date)
                .assign(pitch_type = lambda x : x.pitch_type.replace(pt_dict),
                events = lambda x : x.events.fillna("None")))
